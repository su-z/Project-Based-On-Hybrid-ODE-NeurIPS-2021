...
do
    ...
    do
        ...
        python -u -m experiments.run_simulation --method=${m} --device=${device} --sample=${sample} --path=${model_path} --data_path=${data_path} --eval=y --t0=${t0} "$flagadd" > "results/sample_${sample}_${m}_${t0}.txt"
    done
done
