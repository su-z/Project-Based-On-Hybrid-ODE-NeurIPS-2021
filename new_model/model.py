...
class EncoderLSTM(nn.Module, GaussianReparam):
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, normalize: bool=True, device=None):
        # output dim is the dim of initial condition
        # input dim is observation and action
        my_logging.log("Constructing EncoderLSTM")
    ...
    def forward(self, x, a, mask):
        ...
        # Summary
        # 1. squeeze x (i.e. remove dimensions which have length 1)
        # 2. combine x and a as input
        # 3. pass input through LSTM
        # 4. pass the output (two copies of outputs) to lin and log_var

        # lin stands for the mean of the output
        # log_var stands for the log of the variance
        # So, we get an output with uncertainty instead of a fixed output
        # This is how a variational autoencoder works

        t_max = x.shape[0] # The dimension 0 stands for time.

        x = x.squeeze()

        y_in = torch.cat([x, a], dim=-1)

        # Only x is masked. a is not masked
        mask_in = torch.cat([mask, torch.ones_like(a)], dim=-1)

        hidden = None

        for t in reversed(range(t_max)):
            # This is a slice of only one time point.
            obs = y_in[t: t + 1, ...] * mask_in[t: t + 1, ...]
            # The out part is discarded, unless in the last step (because that is how lstm works)
            out, hidden = self.lstm(obs, hidden)
        out_linear = self.lin(out)
        log_var = self.log_var(out)
...
class RocheODE(nn.Module):
    # Important!! latent space *includes* the expert ODE variables.
    # ml_dim is latent_dim minus the number of expert variables.
    def __init__(self, latent_dim, action_dim, t_max, step_size, ablate=False, device=None, dtype=DTYPE,
                 mix_ml_into_expert: bool = False, duplicate_latent_space: bool = True):
        my_logging.log("Constructing RocheODE!")
        super().__init__()
        ...
        self.mix_ml_into_expert = mix_ml_into_expert
        if self.expanded:
            my_logging.log("self.expanded is true")
            self.ml_net = nn.Sequential(nn.Linear(self.latent_dim, self.ml_dim), nn.Tanh()).to(self.device)

            # An additional network connect ML variables to expert variables
            #self.expert_ml_mix_net = nn.Sequential(nn.Linear(self.latent_dim, self.expert_dim), nn.Tanh()).to(self.device)
            self.expert_ml_mix_net_1 = nn.Sequential(nn.Linear(self.latent_dim, 1), nn.Tanh()).to(self.device)
            self.expert_ml_mix_net_2 = nn.Sequential(nn.Linear(self.latent_dim, 1), nn.Tanh()).to(self.device)
            self.expert_ml_mix_net_3 = nn.Sequential(nn.Linear(self.latent_dim, 1), nn.Tanh()).to(self.device)
            self.expert_ml_mix_net_4 = nn.Sequential(nn.Linear(self.latent_dim, 1), nn.Tanh()).to(self.device)
        else:
    ...
    def forward(self, t, y):
        ...
        #
        # B
        Disease = y[:, 0] # Viral Load, or z4
        ImmuneReact = y[:, 1] # Adaptive Immune Response, or z1
        Immunity = y[:, 2] # Innate Immune Response, or z5
        Dose2 = y[:, 3] # Dex in lung, or z2
        # Does is z3, Dex in plasma. This is given data, not part of ODE

        if not self.ablate:
            Dose = self.dose_at_time(t)

            dxdt1 = ( # d(z4)/dt
                Disease * self.k_disprog # k_DP * z4
                - Disease * Immunity ** self.HillCure * self.k_discure_immunity # z4 * z5^h_C * k_DC
                - Disease * ImmuneReact * self.k_discure_immunereact # k_DC * z4 * z5
            )

            dxdt2 = ( # d(z1)/dt
                Disease * self.k_immune_disease # z4*k_IR
                - ImmuneReact * self.k_immune_off # z1*k_O
                + Disease * ImmuneReact * self.k_immune_feedback # z4*z1*k_PF
                + (ImmuneReact ** self.HillPatho * self.emax_patho) # z1^h_P * E_max
                / (self.ec50_patho ** self.HillPatho + ImmuneReact ** self.HillPatho) # z1^h_P + EC_50^h_P
                - Dose2 * ImmuneReact * self.k_dexa # z2 * z1 * k_Dex ??
            )

            dxdt3 = ImmuneReact * self.k_immunity # d(z5)/dt = z1 * k_1

            # todo: Possible mistake!!!
            dxdt4 = self.kel * Dose - self.kel * Dose2 # d(z2)/dt = k3 * z3 - k2 * z2
        else:
            dxdt1 = ImmuneReact
        ...
        # end of expert part; the following is the ML part.

        if self.expanded:
            my_logging.log("forwarding self.ml_net(y)")
            dmldt = self.ml_net(y) # See explanation for ml_net
            
            # Dynamics due to the mix
            # my_logging.log("forwarding self.expert_ml_mix_net(y)")


            # my_logging.log(dmldt.size(), demldt.size(), dxdt1.size())
            dxdt1_ = dxdt1 + self.expert_ml_mix_net_1(y)[:, 0]
            #my_logging.log(dxdt1_.size(), dxdt1.size())
            dxdt2_ = dxdt2 + self.expert_ml_mix_net_2(y)[:, 0]
            dxdt3_ = dxdt3 + self.expert_ml_mix_net_3(y)[:, 0]
            dxdt4_ = dxdt4 + self.expert_ml_mix_net_4(y)[:, 0]
            
            return torch.cat([dxdt1_[..., None], dxdt2_[..., None], dxdt3_[..., None], dxdt4_[..., None], dmldt], dim=-1)
        else:
...
class DecoderReal(nn.Module):
    ...
    def forward(self, init, a, s):
        ...
        if len(init.shape) == 2:
            # solve ode
            h = odeint(self.ode, init, self.t, method=self.method, options=self.options, rtol=self.rtol, atol=self.atol)
        else:
            h_list = []
...
class NeuralODE(nn.Module):
    ...
    def set_action(self, action):
        # T (Time), B, D (?)
        # B (Batch)
        # Find the maximum value in each batach
        self.dosage = torch.max(action[..., 0], dim=0)[0]

        time_list = []
        for i in range(action.shape[1]):
            # Index of the form (t) where action[t,i,0] != 0
            # Explanation: the times t at which at least one batch gets a positive does
            time = torch.where(action[..., 0][:, i] != 0)[0]
            time = time * self.step_size
            time_list.append(time)
    ...
    def dose_at_time(self, t):
        # self.t = t
        # Maximum dosage * number of batch size i which have a dose at time t
...
class RocheExpertDecoder(nn.Module):
    def __init__(
        ...
        device=None,
        dtype=DTYPE,
        mix_ml_into_expert: bool = True
    ):
        super().__init__()
        my_logging.log("Constructing RocheExpertDecoder")
        my_logging.log(f"obs_dim {obs_dim}, latent_dim {latent_dim}")
        self.mix_ml_into_expert = True
        self.time_dim = int(t_max / step_size)
        self.obs_dim = obs_dim
        self.latent_dim = latent_dim
        self.action_dim = action_dim
        ...
        # ).to(self.device)
        if roche:
            self.ode = RocheODE(latent_dim, action_dim, t_max, step_size, ablate=self.ablate, device=device, mix_ml_into_expert=self.mix_ml_into_expert)
        else:
            self.ode = NeuralODE(latent_dim, action_dim, t_max, step_size, device)

    def forward(self, init: TensorType["batch", "z"], a: TensorType["time", "batch", "a"]):
        self.ode.set_action(a)
        # solve ode
        # h = ode_solver.odesolve(self.ode, init, self.options)
        h = odeint(
            self.ode, init, self.t, 
            rtol=1e-7, atol=1e-8,
        ...
        # generate output
        x_hat = self.output_function(h)
        return x_hat, (h, [self.ode.expert_ml_mix_net_1[0].weight, self.ode.expert_ml_mix_net_2[0].weight, self.ode.expert_ml_mix_net_3[0].weight, self.ode.expert_ml_mix_net_4[0].weight])


class DoubleRocheExpertDecoder(nn.Module):
    def __init__(
        self,
        obs_dim,
        latent_dim,
        ...
        options.update({"t1": t_max + step_size})
        if double_tolarance:
            options.update({"rtol": 2e-7})
            options.update({"atol": 2e-8})
        else:
            options.update({"rtol": 1e-7})
            options.update({"atol": 1e-8})
        options.update({"print_neval": True})
        ...
        self.h_merge_function = nn.Sequential(
    ...
    def forward(self, init1: TensorType["batch", "z"], init2: TensorType["batch", "z"], a: TensorType["time", "batch", "z"]):
        self.ode.set_action(a)
        # solve ode
        # h = ode_solver.odesolve(self.ode, init, self.options)
        h1: TensorType["batch", "z"] = odeint(
            self.ode, init1, self.t, rtol=self.options["rtol"], atol=self.options["atol"], method=self.options["method"]
        )
        h2: TensorType["batch", "z"] = odeint(
            self.ode, init2, self.t, rtol=self.options["rtol"], atol=self.options["atol"], method=self.options["method"]
        )

        h_stack = torch.stack([h1, h2], dim=-1) # Attention!!!
        # generate output
        h = self.output_merge_function(h_stack)
        x_hat = self.output_function(h)
        return x_hat, h
...
class VariationalInference:
    ...
    # 1. data passed to encoder input (x(observation), a(action or medication))
    # Shape of x: (time, batch index, value)
    # 2. Get output (mu, log_var). Pass mu to z (with possible reparameterisation
    # 3. pass z and a to decoder to output something with the same dimension as x, called x_hat, and also h_hat
    # 4.
    ...
    def loss(self, data: dict[str, any]):
        x: TensorType["time", "batch", "x"] = data["measurements"]
        a: TensorType["time", "batch", "a"] = data["actions"]
        mask = data["masks"]
        ...
        penalty = (torch.sum(weights[0] ** 2)+ torch.sum(weights[1] ** 2) + torch.sum(weights[2] ** 2) + torch.sum(weights[3] ** 2))
