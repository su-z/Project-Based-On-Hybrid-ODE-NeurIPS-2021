...
class DataGeneratorRoche:
    def __init__(
        ...
        obs_dim: int, # dimension of observation for each sample
        t_max: int, # Maximum time to integrate ODE to
        step_size: float, # Step size of integrating ODE
        roche_config: sim_config.RochConfig, # Configuration of the RocheODE, that is, parameters of the Roche model
        output_sigma: float, # Variance of output measurement errors
        dose_max: float = 0, # Maximum amount of medicine injected
        latent_dim: int = 4,
        sparsity: float = 0.5,
        ...
        # The mixture coefficients should carry only a small weight
        self.expert_ml_mix_coef_weight = 0.25
        if mix_ml_into_expert:
            # The coefficients of dependence of expert variables on ML latent
            self.expert_ml_mix_coef: Optional[np.ndarray] = (
                np.random.randn(self.latent_dim, self.expert_dim)
                * np.random.binomial(1, 1-self.sparsity, (self.latent_dim, self.expert_dim))
                * self.expert_ml_mix_coef_weight
                / self.latent_dim
            )

        else:
            self.expert_ml_mix_coef: Optional[np.ndarray] = None
    ...
    def solve(self,
              init: np.ndarray, # Initial conditions. In this case, a vector of length self.latent_dim,
                                # generated from exponential distributions
              dose_times, # A sorted list of time when medication is given
              dose_amount):
        # Tackle the easiest variable z3 first
        def dose_at_time(t):
        ...
        # Question: how does a neural network get mixed in ???
        def ode_roche(
            t,
            y,
            HillCure,
            ...
            if self.expanded:
                ...
                if self.expert_ml_mix_coef is not None:
                    # The part of dynamics due to ml states affecting expert states
                    expert_states_ml_mix_part = np.tanh(np.matmul(y, self.expert_ml_mix_coef))
                    return [dxdt1+expert_states_ml_mix_part[0], dxdt2+expert_states_ml_mix_part[1],
                            dxdt3+expert_states_ml_mix_part[2], dxdt4+expert_states_ml_mix_part[3]] + list(ml_states)
                else:
        ...
        res_list = [init] # A list of the state the system goes into at each time interval
    ...
    def get_action(self):
        ...
        for i in range(self.n_sample):
            dose = np.random.choice(self.t_max, size=1, replace=False) # returns a 1d array of shape [1]
            dose_list.append(dose)

        # N, N_dose
        # np.stack is used to convert dose_list into a numpy array
